...
const SYMBOL_SOL_USD = "Crypto.SOL/USD";
...
export class Pyth {
    ...
    async getSolUsdPrice(): Promise<Decimal> {
        ...
        if (!data.symbols.includes(SYMBOL_SOL_USD)) {
            throw new Error(`Symbol not found: ${formatText(SYMBOL_SOL_USD)}`);
        }

        const symbolPrice = data.productPrice.get(SYMBOL_SOL_USD);
        if (!symbolPrice?.aggregate) {
            throw new Error(`Failed to fetch price for ${formatText(SYMBOL_SOL_USD)}`);
        }

        const { price, confidence, status } = symbolPrice.aggregate;
        if (!price || !confidence) {
            throw new Error(`Price current unavailable. Status: ${formatInteger(status)}`);
        }
